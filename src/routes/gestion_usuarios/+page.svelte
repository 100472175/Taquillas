<script lang="ts">
	...
	function add_user() {
		let email = NIA_add + "@alumnos.uc3m.es";
	...
	<TabItem title="Eliminar Rol" class="hover:text-[#3BC4A0]" inactiveClasses="text-gray-500 hover:text-[#3BC4A0] p-4 sm:text-base text-xs">
		<form on:submit={delete_user}>
			<div class="grid grid-cols-1">
				<div>
					<Label class="w-4/5 m-auto text-xl text-[#3BC4A0]">NIA</Label>
					<Input type="text" id="NIA_delete" placeholder="NIA del usuario..." pattern={"100[0-9]{6}"} required class="w-4/5 m-auto" bind:value={NIA_del}/>
				</div>
			</div>
			<div class="w-screen grid grid-cols-1 place-items-center">
				<Button type="submit" class="bg-[#3BC4A0] text-white mt-8 px-8 py-2 text-xl hover:bg-[#3BC4A0]">Eliminar rol</Button>
			</div>
		</form>
	</TabItem>
