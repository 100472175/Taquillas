...
<script lang="ts">
	...
	import { ChartPieSolid, UsersSolid, AnnotationSolid, DrawSquareOutline, HomeSolid } from 'flowbite-svelte-icons';
...
<div class="text-center">
  ...
  <Drawer transitionType="fly" {transitionParams} bind:hidden={hidden2} id="sidebar2">
	<div class="flex items-center">
	  <h5 id="drawer-navigation-label-3" class="text-base font-semibold text-gray-500 uppercase dark:text-gray-400">Menu de Navegación</h5>
	  ...
	  <SidebarWrapper divClass="overflow-y-auto py-4 px-3 rounded dark:bg-gray-800">
		<SidebarGroup>
		  ...
		  <SidebarDropdownWrapper label="Osciloscopios">
			<svelte:fragment slot="icon">
			  <DrawSquareOutline class="w-5 h-5 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" />
			</svelte:fragment>
			<SidebarDropdownItem label="Reserva" href="osciloscopios" on:click={() => hideNavBar()}/>
			<SidebarDropdownItem label="Estado servicio" href="estado_osciloscopios" on:click={() => hideNavBar()}/>
		  ...
		  <SidebarItem label="Encuestas" href='./encuestas' on:click={() => hidden2 = !hidden2}>
			<svelte:fragment slot="icon">
			  <AnnotationSolid class="w-5 h-5 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" />
			</svelte:fragment>
		  </SidebarItem>
		</SidebarGroup>
	  </SidebarWrapper>
